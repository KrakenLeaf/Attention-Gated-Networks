...
Modified class:
    ...
    2. Loads multi modal images  - all of the same projection. Images are assumed to be registered and interpolated 
                                   to the same grid
'''
class CMR3DDataset_MultiClass_MultiProj_infer(data.Dataset):
    def __init__(self, root_dir, split, transform=None, preload_data=True, modalities=['7T_T2']):
            ...
            for jj in range(len(self.image_filenames[0])):  # Per each patient, go over all modalities
                ...
                for ii in range(len(self.image_filenames)):  # Go over all patients, left and right
                    ...
                    if internal_cntr == 0: # First time - should always be T2 axial
                        ...
                        q_dat, _, _ = load_nifti_img(self.image_filenames[ii][jj], dtype=np.float32) # normalize values to [0,1] range
...
class CMR3DDataset_MultiClass_MultiProj_V2(data.Dataset):
    def __init__(self, root_dir, split, transform=None, preload_data=False, rank=0, world_size=1):
