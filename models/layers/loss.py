...
class EnergyLoss(nn.Module):
    def __init__(self, lambda_e=1e-6, T=1, m=-8):
        super(EnergyLoss, self).__init__()
        self.lambda_e = lambda_e
        self.T = T
        self.m = m

        self.zero = torch.tensor([0], requires_grad=False).type(torch.float32).cuda()

    def free_energy(self, input, T=1):
        return -T * torch.logsumexp((input / T), dim=1)

    def forward(self, input):
        loss = 0

        # Term 1:
        #loss += -torch.sum(torch.log(F.softmax(input, dim=1)))

        # Term 2:
        tmp = torch.sum(torch.pow(torch.max(self.free_energy(input, self.T) - self.m, self.zero), 2)) / input.numel()

        loss += self.lambda_e * tmp
...
class TverskyLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        if len(self.alpha) > 1:
            alpha = self.alpha.repeat(batch_size, 1)
        else:
            alpha = self.alpha
        ...
        # -------------
        # Increase STN score times 10, as it will be in the same order of magnitude as the others - assigned value 1
        #tmp[:, 1] = 100 * tmp[:, 1]
        # -------------
...
class jointloss(nn.Module):
    def __init__(self, n_classes, alpha=0.5, beta=0.5, lam=1, hausdorff=-1, lambda_e=1e-6, temp=1, thresh_m=-8):
        ...
        self.lambda_e = lambda_e
        self.energyloss = EnergyLoss(lambda_e=lambda_e, T=temp, m=thresh_m)
        ...
        self.haus_mult_factor = 10 #1000 # 5 - used for the GP project
        self.haus_option = 0 #0 # 0 - Hausdorff distance for the entire GP organ, 1 - for each side and GPi/e separately, 2 - for each label separately, no left/right

    def forward(self, input, target, epoch_cnt):
        # Calculate the multiclass Tversky loss
        # ---------------------------------------------------------------
        score = self.tvloss(input, target) # In fact (1 - Tversky), which we minimize, to maximize the Tversky index
        #print("score = {}, ".format(score))

        # Calculate the energy loss
        # ---------------------------------------------------------------
        if self.lambda_e > 0:
            score += self.energyloss(input)
        ...
        if self.lambda_haus > 0:
            ...
            if self.haus_option == 0: 
                ...
                # Correction:
                inp = F.softmax(input, dim=1)
                inp = torch.sum(inp[:, 1:, ...], axis=1)

                tar = self.one_hot_encoder(target).contiguous()
                #tar = tar[:, 1, ...] + tar[:, 2, ...]
