...
# OS: PyTorch implementation, as a function, not a class - to incorporate in other losses
def av_hausdorff_dist_torch(set1, set2):
    """
    Compute the Averaged Hausdorff Distance function
     between two unordered sets of points (the function is symmetric).
     Batches are not supported, so squeeze your inputs first!
...
class HausdorffLoss(nn.Module):
    ...
    def forward(self, set1, set2):
        ...
        # Modified Chamfer Loss
        term_1 = torch.max(torch.min(d2_matrix, 1))
        term_2 = torch.max(torch.min(d2_matrix, 0))

        res = torch.max(term_1, term_2)
...
if __name__ == '__main__':
    print("Hausdorff distance losses")