...
class DeformConv3D(nn.Module):
    ...
    def forward(self, x, offset):
        ...
        g_000 = (1 + (q_000[..., :N].type_as(p) - p[..., :N])) * (
                    1 + (q_000[..., N:2 * N].type_as(p) - p[..., N:2 * N])) * (
                            1 + (q_000[..., 2 * N:].type_as(p) - p[..., 2 * N:]))  # bilinear kernel (b, h, w, d, N)
        g_111 = (1 - (q_111[..., :N].type_as(p) - p[..., :N])) * (
                    1 - (q_111[..., N:2 * N].type_as(p) - p[..., N:2 * N])) * (
                            1 - (q_111[..., 2 * N:].type_as(p) - p[..., 2 * N:]))
