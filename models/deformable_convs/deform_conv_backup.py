...
class DeformConv3D(nn.Module):
    ...
    def forward(self, x, offset):
        ...
        q_001 = torch.cat([q_000[..., :N], q_000[..., N:2 * N], q_111[..., 2 * N:]], dim=-1).cuda(1)
        q_010 = torch.cat([q_000[..., :N], q_111[..., N:2 * N], q_000[..., 2 * N:]], dim=-1).cuda(1)
        q_011 = torch.cat([q_000[..., :N], q_111[..., N:2 * N], q_111[..., 2 * N:]], dim=-1).cuda(1)
        q_100 = torch.cat([q_111[..., :N], q_000[..., N:2 * N], q_000[..., 2 * N:]], dim=-1).cuda(1)
        q_101 = torch.cat([q_111[..., :N], q_000[..., N:2 * N], q_111[..., 2 * N:]], dim=-1).cuda(1)
        q_110 = torch.cat([q_111[..., :N], q_111[..., N:2 * N], q_000[..., 2 * N:]], dim=-1).cuda(1)

        # Interpolation kernel - x(q) (Eq. 4)
        # -----------------------------------
        # bilinear kernel (b, h, w, d, N)
        g_000 = (1 + (q_000[..., :N].type_as(p) - p[..., :N])).cuda(0) * \
                (1 + (q_000[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(0) * \
                (1 + (q_000[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(0)
        g_111 = (1 - (q_111[..., :N].type_as(p) - p[..., :N])).cuda(0) * \
                (1 - (q_111[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(0) * \
                (1 - (q_111[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(0)
        g_001 = (1 + (q_000[..., :N].type_as(p) - p[..., :N])).cuda(1) * \
                (1 + (q_000[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(1) * \
                (1 - (q_111[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(1)
        g_010 = (1 + (q_000[..., :N].type_as(p) - p[..., :N])).cuda(1) * \
                (1 - (q_111[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(1) * \
                (1 + (q_000[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(1)
        g_011 = (1 + (q_000[..., :N].type_as(p) - p[..., :N])).cuda(2) * \
                (1 - (q_111[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(2) * \
                (1 - (q_111[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(2)
        g_100 = (1 - (q_111[..., :N].type_as(p) - p[..., :N])).cuda(2) * \
                (1 + (q_000[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(2) * \
                (1 + (q_000[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(2)
        g_101 = (1 - (q_111[..., :N].type_as(p) - p[..., :N])).cuda(2) * \
                (1 + (q_000[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(2) * \
                (1 - (q_111[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(2)
        g_110 = (1 - (q_111[..., :N].type_as(p) - p[..., :N])).cuda(2) * \
                (1 - (q_111[..., N:2 * N].type_as(p) - p[..., N:2 * N])).cuda(2) * \
                (1 + (q_000[..., 2 * N:].type_as(p) - p[..., 2 * N:])).cuda(2)
