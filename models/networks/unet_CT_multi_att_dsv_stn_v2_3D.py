...
import torch
from .utils import UnetConv3, UnetUp3_CT, UnetGridGatingSignal3, UnetDsv3, STNblock, STNblock_v2
import torch.nn.functional as F
from models.networks_other import init_weights
from models.layers.grid_attention_layer import GridAttentionBlock3D


class unet_CT_multi_att_dsv_stn_v2_3D(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(unet_CT_multi_att_dsv_stn_v2_3D, self).__init__()
        ...
        self.stn_t1 = STNblock_v2(n_channels=1, target_grid_size=torch.Size(dataDims), type='3d')
        #self.stn_fa = STNblock_v2(n_channels=1, target_grid_size=torch.Size(dataDims), type='3d')
        ...
        self.conv1 = UnetConv3(self.in_channels, filters[0], self.is_batchnorm, kernel_size=(19,19,19), padding_size=(9,9,9))
        self.maxpool1 = nn.MaxPool3d(kernel_size=(2, 2, 2))

        #self.conv2 = UnetConv3(filters[0], filters[1], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        self.conv2 = UnetConv3(filters[0], filters[1], self.is_batchnorm, kernel_size=(11,11,11), padding_size=(5,5,5))
        self.maxpool2 = nn.MaxPool3d(kernel_size=(2, 2, 2))

        #self.conv3 = UnetConv3(filters[1], filters[2], self.is_batchnorm, kernel_size=(3,3,3), padding_size=(1,1,1))
        self.conv3 = UnetConv3(filters[1], filters[2], self.is_batchnorm, kernel_size=(5,5,5), padding_size=(2,2,2))
    ...
    def forward(self, inputs_mult):

        # inputs_split = torch.split(inputs_mult, split_size_or_sections=1, dim=1)
        #
        # # Forward the modalitites through the STN blocks
        # stn_t2_o = inputs_split[0] #self.stn_t1(inputs_split[0]) # T2 Axial
        # stn_t1_o = self.stn_t1(inputs_split[1]) # T1 Axial
        # stn_fa_o = self.stn_t1(inputs_split[2]) # FA Axial
        #
        # #####
        # # import matplotlib.pyplot as plt
        # # import numpy as np
        # # q = inputs_split[2].cpu().numpy(); plt.figure(); plt.imshow(np.squeeze(q[0, 0, :, :, 60]));
        # # q = stn_fa_o.cpu().numpy(); plt.figure(); plt.imshow(np.squeeze(q[0, 0, :, :, 60])); plt.show()
        #
