...
'''
OS: STN based segmentation network for both axial and coronal data, with s skip connection 
'''
class unet_CT_multi_att_dsv_stn_3D(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(unet_CT_multi_att_dsv_stn_3D, self).__init__()
        ...
        self.stn_axial = STNblock_v2(n_channels=1, target_grid_size=torch.Size(dataDims), type='3d')
        self.stn_coronal = STNblock(n_channels=1, target_grid_size=torch.Size(dataDims), type='3d')
        ...
        self.final = nn.Conv3d(in_channels + n_classes*4, n_classes, 1)
    ...
    def forward(self, inputs_ax, inputs_cor):
        inputs_split = torch.split(inputs_ax, split_size_or_sections=1, dim=1)

        # Forward the modalitites through the STN blocks
        stn_t2_a = inputs_split[0]  # self.stn_t1(inputs_split[0]) # T2 Axial
        stn_t1_a = self.stn_axial(inputs_split[1])  # T1 Axial
        stn_fa_a = self.stn_axial(inputs_split[2])  # FA Axial
        stn_t2_c = self.stn_coronal(inputs_cor)  # T2 Coronal

        # Concatenate the axial and transformed coronal volumes and pass on to the Unet
        inputs = torch.cat([stn_t2_a, stn_t1_a, stn_fa_a, stn_t2_c], 1)
        ...
        dsv2 = self.dsv2(up2)
        dsv1 = self.dsv1(up1)
        final = self.final(torch.cat([inputs,dsv1,dsv2,dsv3,dsv4], dim=1))
...
'''
OS: remove the "_old_version" from the class name to use the STN based axial and coronal network without the skip 
connection
'''
class unet_CT_multi_att_dsv_stn_3D_old_version(nn.Module):
