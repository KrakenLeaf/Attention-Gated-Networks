...
class transformer_registration_3D(nn.Module):

    def __init__(self, vol_size, enc_nf, dec_nf):
        super(transformer_registration_3D, self).__init__()
        self.vol_size = vol_size
        self.enc_nf = enc_nf
        self.dec_nf = dec_nf
        self.voxel_morph = cvpr2018_net(vol_size, enc_nf, dec_nf)

    def forward(self, I, J):
        reg_vol = self.voxel_morph(I, J)
        return reg_vol
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):   
        ...
        """
        # New locations are the regular grid + learnt displacements XYZ
