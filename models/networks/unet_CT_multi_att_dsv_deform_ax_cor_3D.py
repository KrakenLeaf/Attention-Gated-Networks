...
class unet_CT_multi_att_dsv_deform_ax_cor_3D(nn.Module):
    ...
    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3,
        ...
        super(unet_CT_multi_att_dsv_deform_ax_cor_3D, self).__init__()
        ...
        # downsampling
        self.conv1 = UnetConv3(self.in_channels, filters[0], self.is_batchnorm, kernel_size=11, padding_size=15, init_dilation=3) # Option 1
        self.maxpool1 = nn.MaxPool3d(kernel_size=(2, 2, 2))

        self.conv2 = UnetConv3(filters[0], filters[1], self.is_batchnorm, kernel_size=9, padding_size=8, init_dilation=2) # Option 1
        self.maxpool2 = nn.MaxPool3d(kernel_size=(2, 2, 2))

        #self.conv3 = UnetConv3(filters[1], filters[2], self.is_batchnorm, kernel_size=5, padding_size=2, init_dilation=1) # Option 1
        self.conv3 = UnetConv3_deformable(filters[1], filters[2], self.is_batchnorm, kernel_size=5, padding_size=2)  # Option 2 - use this instead of 1
        ...
        # upsampling
        self.up_concat4 = UnetUp3_CT_deformable(filters[4], filters[3], init_kernel=3, init_padding=1, is_batchnorm=is_batchnorm) # Option 1+2
        self.up_concat3 = UnetUp3_CT_deformable(filters[3], filters[2], init_kernel=3, init_padding=1, is_batchnorm=is_batchnorm) # Option 1+2
        self.up_concat2 = UnetUp3_CT(filters[2], filters[1], init_kernel=5, init_padding=2, is_batchnorm=is_batchnorm)
        self.up_concat1 = UnetUp3_CT(filters[1], filters[0], init_kernel=7, init_padding=3, is_batchnorm=is_batchnorm)
    ...
    def forward(self, vol_ax, vol_cor):
        inputs = torch.cat((vol_ax, vol_cor), axis=1) # Concatentate the axial and coronal volumes along the channel dim
